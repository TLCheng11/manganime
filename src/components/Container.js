...
function Container({ manganime, setSelectedItem, reset, setReset }) {
  ...
  const pages =
    manganime.length % 4 !== 0
      ? manganime.slice(4, -(manganime.length % 4))
      : manganime.slice(4);
    ...
    <div className="container">
      <div className="cover">
        <div className="book">
          ...
          <div
            className="book__page book__page--last"
            ...
            <div className="itemsContainer">
              {manganime.length === 0 ? (
                ...
                manganime
                  .slice(
                    manganime.length -
                      (manganime.length % 4 === 0 ? 4 : manganime.length % 4)
                  )
                  .map((item) => (
                    <Thumbnail
                      key={item.id}
